...
it('validates required fields when storing an order', function () {
    $this->post(route('order.store', ['locale' => 'id']), [])
...
it('creates an order with the computed total', function () {
    ...
    $response = $this->post(route('order.store', ['locale' => 'id']), [
    ...
    $response->assertRedirect(route('order.success', ['locale' => 'id', 'orderNumber' => $order->order_number]));
});
...
it('prevents duplicate orders within five minutes', function () {
    ...
    $this->post(route('order.store', ['locale' => 'id']), $payload);
    $existing = Order::first();

    $this->post(route('order.store', ['locale' => 'id']), $payload)
        ->assertRedirect(route('order.success', ['locale' => 'id', 'orderNumber' => $existing->order_number]));
...
it('finds an order on the track page', function () {
    ...
    $this->get(route('order.track', ['locale' => 'id', 'order_number' => $order->order_number]))
        ->assertOk()
...
it('shows a not found alert on the track page', function () {
    $this->get(route('order.track', ['locale' => 'id', 'order_number' => 'ORD-999999999']))
        ->assertOk()
